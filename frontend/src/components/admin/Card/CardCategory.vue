  ...
  <div class="bg-white dark:bg-dark-sidebar rounded-lg dark:border group hover:duration-700">
    <div class="p-3 h-full flex flex-wrap justify-center">
      <div class="w-full relative">
        ...
        <el-tooltip
          class="box-item"
          :effect="darkModeStore.darkMode? 'light': 'dark'"
          :content="props.category.status=== 'active' ? 'Dừng kích hoạt' : 'Kích hoạt'"
          placement="top-start"
        >
          <label class="inline-flex items-center cursor-pointer absolute p-3 top-0 right-0">
            ...
            type="checkbox"
            :checked="isActive"
            @change="toggleStatus(props.category.id, props.category.status)"
            class="sr-only peer">
        ...
        <div class="py-4">
          ...
          <ItemsChild 
          v-for="(CategoryItemChild, index) in props.category.children" 
          :key="index" 
