...
import { SidebarItemChildren } from '@/interfaces/admin.interface';
import { SidebarItemChildren2 } from '@/interfaces/admin.interface';
...
<template>
  <div>
      ...
      :class="{'mt-0 pl-0 p-4': sidebarStore.isSidebarOpen}"
      >
        <template v-for="(childItem, index) in props.items" :key="index"> 
          <li>
            <router-link
              ...
              @click.prevent="handleDropdownClick(index)"
                :class="{
            ...
            >
              {{ childItem.label }}
              <ChevronRightIcon v-if="childItem.children"
              class="h-5 w-5 right-2 absolute"
