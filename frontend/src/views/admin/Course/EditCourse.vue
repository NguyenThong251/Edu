<template>
          ...
          <form @submit.prevent="submitFormEdit">
            <div class="flex justify-between gap-2 lg:flex-row flex-col">
              <div class="flex gap-5">
                <ButtonSecondary :icon="ArrowTopRightOnSquareIcon" link="#" title="Xem trước giao diện"
                  customStyle="!flex-row-reverse font-normal" />
                <ButtonSecondary :icon="ArrowTopRightOnSquareIcon" link="#" title="Trình phát khoá học"
                  customStyle="!flex-row-reverse font-normal" />
              </div>
              <div class="flex">
            ...
            <div class="pt-4 pb-8">
              <el-tabs :tab-position="tabPosition" style="height: 200px"
                ...
                <el-tab-pane>
                  ...
                  <div class="px-2">
                    <div class="my-2 flex gap-2">
                      ...
                      <ButtonPrimarySm title="Thêm chương" dialogVisible="dialogAddnewSection" link="#"
                        @click="addChapter" />
                      <!-- button dialog 2 new lesson -->
                      <template v-if="section.length > 0">
                        <ButtonPrimarySm title="Thêm bài học" dialogVisible="dialogAddnewLesson" link="#"
                          @click="addLesson" />
                        <!-- button dialog 3 new quiz -->
                        <ButtonPrimarySm title="Thêm quiz" dialogVisible="dialogAddnewQuiz" link="#" @click="addQuiz" />
                        <!-- button dialog 4 sort section -->
                        <ButtonPrimarySm title="Sắp xếp chương" dialogVisible="dialogSortSection" link="#"
                          @click="sortSection" />
                      </template>
                    </div>
                    <template v-if="section.length == 0">
                    ...
                    <template v-else>
                      <!-- Dropdown items -->
                      <div class="mt-6">
                        <template v-for="(itemSection, index) in section.reverse()" :key="itemSection.id">
                          <el-collapse class="mb-2" @change="ActiveToggle">
                            <el-collapse-item name="1" class="group style-input hover:opacity-100" @click.stop>
                              <template #title>
                                ...
                                <div
                                  ...
                                  <ButtonSecondarySm link="#" :icon="PencilIcon"
                                    @click.prevent="EditChapter(itemSection.id)" />
                                  <ButtonSecondarySm link="#" :icon="TrashIcon"
                                    @click.prevent="handleDeleteSection(itemSection.id)" />
                                </div>
                              </template>
                              <template v-if="
                              ...
                              ">
                                <template v-for="itemLecture in itemSection.lectures" :key="itemLecture.id">
                ...
                <el-tab-pane>
                  ...
                  <div class="px-2">
                    <InputGroup label="Tiêu đề" required="*" inputId="name" customsClass="flex flex-row items-center"
                      customsClassChild="basis-2/12" customsClassChild2="basis-10/12 w-full"
                      v-model="formDataEditCourse.title" />

                    <DescriptionGroup label="Mô tả ngắn" inputId="description" inputPlaceHoder="Nhập mô tả..."
                      customsClass="flex flex-row items-center" customsClassChild="basis-2/12"
                      customsClassChild2="basis-10/12 w-full" v-model="formDataEditCourse.short_description" />
                    <CkeditorGroup label="Nội dung" customsClass="flex flex-row items-center"
                      customsClassChild="basis-2/12" customsClassChild2="basis-10/12 w-full"
                      v-model="formDataEditCourse.description" />

                    <SelectGroup inputPlaceHoder="Chọn danh mục cho khoá học" required="*" label="Danh mục"
                      customsClass="flex flex-row items-center" customsClassChild="basis-2/12"
                      customsClassChild2="basis-10/12 w-full" v-model="formDataEditCourse.category_id"
                      :optionsData="categories" />
                    <SelectGroup inputPlaceHoder="Chọn cấp độ cho khoá học" required="*" label="Cấp độ"
                      customsClass="flex flex-row items-center" customsClassChild="basis-2/12"
                      customsClassChild2="basis-10/12 w-full" :optionsData="courseLevels"
                      v-model="formDataEditCourse.level_id" />
                    <SelectGroup inputPlaceHoder="Chọn ngôn ngữ cho khoá học" required="*" label="Ngôn ngữ"
                      customsClass="flex flex-row items-center" customsClassChild="basis-2/12"
                      customsClassChild2="basis-10/12 w-full" :optionsData="languages"
                      v-model="formDataEditCourse.language_id" />
                    <RadioGroup label="Chọn" required="*" customsClass="flex flex-row items-center"
                      customsClassChild="basis-2/12" customsClassChild2="basis-10/12 w-full">
                      <div class="mb-2 ml-4">
                ...
                <el-tab-pane>
                  ...
                  <div class="px-2">
                    <RadioGroup label="Loại giá" customsClass="flex flex-row items-center"
                      customsClassChild="basis-2/12" customsClassChild2="basis-10/12 w-full">
                    ...
                    <template :class="{ block: paid == 1, hidden: paid == 0 }"
                      ...
                      <InputGroup label="Giá (đơn vị vnđ)" required="*" inputPlaceHoder="Nhập giá của khoá học"
                        customsClass="flex flex-row items-center" customsClassChild="basis-2/12"
                        customsClassChild2="basis-10/12 w-full" v-model="formDataEditCourse.price" />
                      <div v-show="checked2" class="relative">
                        <el-tooltip class="box-item" effect="dark" content="Chuyển sang phần trăm" placement="top">
                          <ArrowPathIcon class="w-5 absolute top-0 left-[100px] bold color-primary" />
                        </el-tooltip>
                        <InputGroup label="Giá giảm" required="*" inputPlaceHoder="Nhập giá giảm của khoá học"
                          customsClass="flex" customsClassChild="basis-2/12" customsClassChild2="basis-10/12 w-full"
                          v-model="formDataEditCourse.sale_value" />
                      </div>
                      <div class="mt-1 flex justify-end">
                        <el-checkbox class="basis-10/12" v-model="checked2"
                          label="Click nếu khoá học này có sử dụng mã giảm giá" size="large" />
                      </div>
                    </template>
                  </div>
                </el-tab-pane>
                ...
                <el-tab-pane>
                  ...
                  <div class="px-2">
                    <RadioGroup label="Faq" required="*" inputPlaceHoder="Nhập đường dẫn video"
                      customsClass="flex flex-row" customsClassChild="basis-2/12"
                      customsClassChild2="basis-10/12 w-full">
                      <div class="">
                        <InputItems :icon="PlusIcon" inputPlaceHoder="Câu hỏi" />
                        <textarea :id="inputId" :name="inputId" placeholder="Trả lời" :value="value"
                          :v-model="modelValue" rows="2" class="block p-2.5 input-style mt-2"
                          :class="customsClassChild2">
        </textarea>
                      </div>
                    </RadioGroup>
                    <RadioGroup label="Yêu cầu" required="*" customsClass="flex flex-row" customsClassChild="basis-2/12"
                      customsClassChild2="basis-10/12 w-full">
                      <div class="">
                    ...
                    <RadioGroup label="Kết quả" required="*" customsClass="flex flex-row" customsClassChild="basis-2/12"
                      customsClassChild2="basis-10/12 w-full">
                      <div class="">
                ...
                <el-tab-pane>
                  ...
                  <div class="px-2">
                    <UploadGroup label="Hình ảnh thumbnail" inputPlaceHoder="Nhập giá của khoá học đã giảm"
                      customsClass="flex flex-row items-center" customsClassChild="basis-2/12"
                      customsClassChild2="basis-10/12 w-full" :handlePreviewImg="handlePreviewImg" />
                    <UploadGroup label="Hình ảnh banner" inputPlaceHoder="Nhập giá của khoá học đã giảm"
                      customsClass="flex flex-row items-center" customsClassChild="basis-2/12"
                      customsClassChild2="basis-10/12 w-full" />
                    <div class="border mt-7 mb-5"></div>

                    <RadioGroup label="Video intro" customsClass="flex flex-row items-center"
                      customsClassChild="basis-2/12" customsClassChild2="basis-10/12 w-full">
                    ...
                    <UploadGroup label="Video file" required="*" customsClass="flex flex-row items-center"
                      customsClassChild="basis-2/12" customsClassChild2="basis-10/12 w-full"
                      :class="{ hidden: linkVideo == 1 }" />
                    <InputGroup label="Video link" inputId="video" required="*" inputPlaceHoder="Nhập đường dẫn video"
                      customsClass="flex flex-row items-center" customsClassChild="basis-2/12"
                      customsClassChild2="basis-10/12 w-full" :class="{ hidden: linkVideo == 0 }" />
                  </div>
                </el-tab-pane>
                ...
                <el-tab-pane>
                  ...
                  <div class="px-2">
                    <InputGroup label="Tiêu đề SEO (Tên khoá học)" required="*" inputId="name"
                      inputPlaceHoder="Nhập tiêu đề SEO" />
                    <InputOptionGroup label="Từ khoá SEO" required="*" inputId="name"
                      inputPlaceHoder="Nhập từ khoá seo" />
                    <DescriptionGroup label="Mô tả SEO" required="*" inputId="name"
                      inputPlaceHoder="Nhập từ khoá seo" />
                    <InputGroup label="Robot SEO" required="*" inputId="name" inputPlaceHoder="Robot SEO" />
                    <InputGroup label="URL SEO" required="*" inputId="name"
                      inputPlaceHoder="https://demo.edunity.com/edunity-laravel" />
                    <InputGroup label="Tiêu đề Og" required="*" inputId="name"
                      inputPlaceHoder="Responsive Web Design Essentials - HTML5 CSS Bootstrap" />
                    <DescriptionGroup label="Mô tả Og" required="*" inputId="name"
  ...
  <!-- Dialog thêm chương -->
  <DialogArea :dialogVisible="dialogAddnewSection" title="Thêm chương mới" @close="dialogAddnewSection = false"
    :submitForm="handelFormSection">
    <InputGroup label="Tên chương" inputId="nameSection" inputPlaceHoder="Nhập tên chương"
      v-model="formDataAddSection.name" />
  </DialogArea>
  <!-- End Dialog thêm chương -->
  <!-- Dialog Chỉnh sửa chương -->
  <DialogArea :dialogVisible="dialogEditSection" title="Chỉnh sửa chương" @close="dialogEditSection = false"
    :submitForm="() => handleEditSection(selectedSectionId)">
    <InputGroup label="Chỉnh sửa tên chương" inputId="nameEditSection" inputPlaceHoder="Nhập tên chương"
      v-model="formDataEditSection.name" />
  </DialogArea>
  <!-- End Dialog Chỉnh sửa chương -->

  <!-- Dialog thêm bài học -->
  <DialogArea :dialogVisible="dialogAddnewLesson" title="Thêm bài học mới" @close="dialogAddnewLesson = false">
    ...
    <InputGroup inputId="videoLesson" label="Video bài học" inputPlaceHoder="Nhập đường dẫn video" />
  </DialogArea>
  <!-- Dialog thêm quiz -->
  <DialogArea :dialogVisible="dialogAddnewQuiz" title="Thêm quiz mới" @close="dialogAddnewQuiz = false">
    <InputGroup inputId="nameQuiz" label="Tên quiz" inputPlaceHoder="Nhập tên quiz" />
    <InputGroup inputId="timeQuiz" label="Thời gian làm bài" inputPlaceHoder="Nhập thời gian làm bài" />
  ...
  <DialogArea :dialogVisible="dialogSortSection" title="Sắp xếp các chương" @close="dialogSortSection = false">
    <el-input style="max-width: 600px" class="cursor-move py-4" placeholder="Chương 1">
...
import type { Option, TListCategories } from '@/interfaces'
